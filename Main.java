...
public class Main {
    ...
    static class World {
        ...
        public int increaseAge(int tAge) {
            tAge++;
            return tAge;
        }
        ...
        public Creature createCreature() {
            ...
            creature.name = names[(int) (Math.random() * names.length)];
            creature.age = 0;
    ...
    static class Creature {
        ...
        public boolean eat(boolean hunger) {
            hunger = false;
            return hunger;
        }
    }

    static class Player extends Creature {
    ...
    public static void main(String[] args) {
        ...
        Creature creatureNameOne = world.createCreature();
        residents.add(creatureNameOne);
        System.out.println("Welcome to the world.");
        System.out.println("If a creature is hungry, there is a chance that they die the next year.");
        Scanner scnr = new Scanner(System.in);
        while (!userQuit.equals("Q") && residents.size() > 0) {
            for (int i = 0; i < 3; i++) {
                Creature creatureName = world.createCreature();
                residents.add(creatureName);
            }
            int food = (int) ((Math.random() * residents.size() + 1) + ((int) (residents.size() / 2)));
            world.est++;
            System.out.println("Year " + world.est);
            System.out.println("Residents: " + residents.size());
            for (int i = 0; i < residents.size(); i++) {
                residents.get(i).isHungry = true;
                System.out.println(residents.get(i).name + ", age " + residents.get(i).age);
                residents.get(i).age++;
                if (residents.get(i).isHungry == true && residents.get(i).hungerLength >= 1) {
                    System.out.println(residents.get(i).name + " has been hungry for " + residents.get(i).hungerLength
                            + " years.");
                }
                if (residents.get(i).hungerLength >= 1) {
                    int chanceDeath = (int) (Math.random() * 2);
                    if (chanceDeath == 0) {
                        residents.get(i).die();
                        residents.remove(i);
                        residents.get(i).hungerLength = 0;
                    }
                }
                if (food >= 1) {
                    food--;
                    residents.get(i).isHungry = false;
                }
                residents.get(i).hungerLength++;
            }
            System.out.println(
                    "Please press any letter on the keyboard to move on to the next year. Or type \"Q\" to quit.");
            userQuit = scnr.next();
        }
        if (residents.size() <= 0) {
            System.out.println("There are no more creatures.");
        }
        scnr.close();
    }
}
